...
library(ggthemes)
library(ggprism)
setwd=("C:\\Users\\anxin\\Desktop\\integrated_model\\GSVA") 
...
p <- ggplot(data = dat_plot,aes(x = id, y = tvalue, fill = g)) +
  ...
  geom_text(data = dat_plot, aes(x=id, y = lable_xloc, label = label_wrap, hjust = lable_hjust),
            color=dat_plot$color, size = 2.5)

p
write.csv(dat_plot,file="GSVAtest-neural stress.csv",quote=FALSE)
ggsave("GSVAtest_neural stress.pdf",p,device="pdf",width=11,height=10)
