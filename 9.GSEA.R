...
setwd("C:\\Users\\anxin\\Desktop\\integrated_moel")
...
write.table(kkTab,file="GSEA.GOresult.txt",sep="\t",quote=F,row.names = F)
...
if(nrow(kkUp)>=termNum){
  ...
  pdf(file="GSEA.GO_AA_Top10.pdf", width=10, height=8)
  print(gseaplot)
  dev.off()
}
...
if(nrow(kkDown)>=termNum){
  ...
  gseaplot=gseaplot2(kk, 
                     ...
                     rel_heights = c(2.5, 0.5, 1))
  pdf(file="GSEA.GO_con_Top10.pdf", width=10, height=8)
  print(gseaplot)
  dev.off()
}
